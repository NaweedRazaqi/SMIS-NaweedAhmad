    ...
    public class GetScreensHandler : IRequestHandler<GetScreens, List<Screen>>
    {
        ...
        public async Task<List<Screen>> Handle(GetScreens request, CancellationToken cancellationToken)
        {
            ...
            if (request.ModuleID.HasValue)
            {
                ...
                return await _context.Screens.Where(c => c.ParentId == null && c.ModuleId == request.ModuleID & c.Module.IsActive == true).OrderBy(c => c.Sorter).ToListAsync(cancellationToken);
            }
            else if (request.ModuleIDNotIn != null)
            {
                return await _context.Screens.Where(s => s.ModuleId != request.ModuleIDNotIn & s.Module.IsActive == true).OrderBy(c => c.Sorter).ToListAsync(cancellationToken);
            }
            else if (request.ID == null && request.ModuleID == null && request.ModuleIDNotIn == null)
            {
                ...
                return await _context.Screens.Where(S => S.Module.IsActive == true).ToListAsync(cancellationToken);
            }
            else
            {
                return await _context.Screens.Where(s => s.Id == request.ID && s.Module.IsActive == true).ToListAsync(cancellationToken);
            }
